...
uv run python examples/05_messages_list.py
"""
...
def main() -> None:
    ...
    s = u.setup(
        client,
        agent_name="Personal assistant (messages list)",
        agent_description="Personal assistant using full message list history + RAG.",
        prompt_name="Personal Assistant Messages List",
        prompt_content=SYSTEM,
        session_external_id="personal-assistant-messages-list",
        session_title="Personal assistant — messages list",
    )
    ...
    try:
        while True:
            ...
            stored = client.message.list(s.agent_id, s.session_id, limit=100)
            reply, usage = u.ask(
                system=s.system,
                user=text,
                history=u.history_from_messages(stored.messages),
