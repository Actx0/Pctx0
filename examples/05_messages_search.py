...
uv run python examples/05_messages_search.py
"""
...
def main() -> None:
    ...
    s = u.setup(
        client,
        agent_name="Personal assistant (messages search)",
        agent_description="Personal assistant using message search history + RAG.",
        prompt_name="Personal Assistant Messages Search",
        prompt_content=SYSTEM,
        session_external_id="personal-assistant-messages-search",
        session_title="Personal assistant — messages search",
    )
    ...
    try:
        while True:
            ...
            hits = client.message.search(s.agent_id, s.session_id, query=text, limit=5)
            reply, usage = u.ask(
                system=s.system,
                user=text,
                history=u.history_from_message_hits(hits.results),
