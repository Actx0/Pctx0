...
uv run python examples/06_memories_list.py
"""
...
def main() -> None:
    ...
    s = u.setup(
        client,
        agent_name="Personal assistant (memories list)",
        agent_description="Personal assistant using full memory list history + RAG.",
        prompt_name="Personal Assistant Memories List",
        prompt_content=SYSTEM,
        session_external_id="personal-assistant-memories-list",
        session_title="Personal assistant — memories list",
    )
    ...
    try:
        while True:
            ...
            stored = client.memory.list(s.agent_id, s.session_id, limit=100)
            reply, usage = u.ask(
                system=s.system,
                user=text,
                history=u.history_from_memories(stored.memories),
